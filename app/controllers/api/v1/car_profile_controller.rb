class Api::V1::CarProfileController < ApplicationController

    before_action :authenticate_user_api! , except: [:show]
    ...
    def create
        ...
        @car.image.attach params[:carProfile][:image]
        @car.user = current_user
        if @car.save
            render json: { message: "Your car is added successfully!", status: 200 }
        else
    ...
    def car_params
        params.require(:carProfile).permit(
            :model,
            :content,
            :year,
            :mileage,
